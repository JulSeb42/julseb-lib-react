/*=============================================== Skeleton tests ===============================================*/

import { SkeletonCard, Skeleton } from "../Skeleton"
import { cssVariables } from "../../.."

describe("<Skeleton />", () => {
    it("renders <Skeleton /> component", () => {
        cy.mount(
            <SkeletonCard data-testid="testid">
                <Skeleton data-testid="testid.Skeleton" />
            </SkeletonCard>
        )

        const card = cy.dataTest("testid")

        card.should("exist")
        cy.dataTest("testid.Shine").should("not.exist")
        card.children().should("have.length", 1)
    })

    it("has default properties", () => {
        cy.mount(<Skeleton data-testid="testid" />)
        cy.dataTest("testid").should(
            "have.css",
            "background-color",
            cssVariables.colors.light["gray-100"].rgb
        )
    })

    it("renders a shine when using the `isShiny` prop", () => {
        cy.mount(<SkeletonCard data-testid="testid" isShiny />)

        cy.dataTest("testid.Shine").should("exist")
    })

    it("renders a default border when the width is set to 1", () => {
        cy.mount(
            <SkeletonCard
                data-testid="testid"
                border={{ width: 1 }}
                borderRadius="xs"
            />
        )

        cy.dataTest("testid")
            .should("have.css", "border-width", "1px")
            .should(
                "have.css",
                "border-color",
                cssVariables.colors.light["gray-200"].rgb
            )
            .should("have.css", "border-style", "solid")
            .should("have.css", "border-radius", cssVariables.radiuses.xs)
    })

    it("renders a default border when the color is set to gray-300, and set border radius", () => {
        cy.mount(
            <SkeletonCard
                data-testid="testid"
                border={{ color: "gray-300" }}
                borderRadius={{
                    topLeft: "xxl",
                    topRight: "xs",
                    bottomLeft: "m",
                    bottomRight: "l",
                }}
                padding={{
                    topBottom: "xxl",
                    left: "s",
                    right: "m",
                }}
            />
        )

        cy.dataTest("testid")
            .should("have.css", "border-width", "1px")
            .should(
                "have.css",
                "border-color",
                cssVariables.colors.light["gray-300"].rgb
            )
            .should("have.css", "border-style", "solid")
            .should(
                "have.css",
                "border-top-left-radius",
                cssVariables.radiuses.xxl
            )
            .should(
                "have.css",
                "border-top-right-radius",
                cssVariables.radiuses.xs
            )
            .should(
                "have.css",
                "border-bottom-left-radius",
                cssVariables.radiuses.m
            )
            .should(
                "have.css",
                "border-bottom-right-radius",
                cssVariables.radiuses.l
            )
            .should("have.css", "padding-top", cssVariables.spacers.xxl)
            .should("have.css", "padding-bottom", cssVariables.spacers.xxl)
            .should("have.css", "padding-left", cssVariables.spacers.s)
            .should("have.css", "padding-right", cssVariables.spacers.m)
    })

    it("renders set width and height", () => {
        cy.mount(<SkeletonCard data-testid="testid" width={300} height={400} />)

        cy.dataTest("testid").invoke("width").should("equal", 300)
        cy.dataTest("testid").invoke("height").should("equal", 400)
    })
})
